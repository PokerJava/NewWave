...
public class Out_SDF_GetResourceInventory {
    public EquinoxRawData messageBuilder(AbstractAF abstractAF, AFInstance afInstance, AFSubInstance afSubIns) {
        ...
        Config.setResourceInventoryHashMap(new HashMap<>()); //clear memory before get new values
        String message = "";
        ...
        //-- Timeout --//
        String timeout = Config.getFormatDateWithMiTz().print(new DateTime().plusSeconds(Config.getSdfInventoryTimeout()));
        afSubIns.setSubTimeout(timeout);
        afSubIns.setSubTimeoutArray(timeout);
        
        //-- Stat --//
       // StatUtils.incrementStats(abstractAF, StatUtils.PEGAZUS.pgz_sdf_send_GET_resourceinventory_success_req);
        
        EStats statsOut = EStats.APP_SEND_SDF_GET_W_SDF_GETRESOURCEINVENTORY_REQUEST;
