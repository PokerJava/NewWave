...
public class Out_SDF_CommitQuotaInfra {
	public EquinoxRawData messageBuilder(AbstractAF abstractAF, AFInstance afInstance, AFSubInstance afSubIns) {
		...
		map.put(EEqxMsg.DESTINATION.getEqxMsg(), Config.getSdfCommitquotaInterface());
		String url = Config.getUrlSdfCommitquota();
		...
		String invoke = new AFUtils().invokeGenerator(abstractAF, afSubIns.getSubInitInvoke(), afSubIns.getSubInitCmd(),
				afSubIns.getSubNextState(), afSubIns.getSubInstanceNo(), "$TaskNo");
		...
		// -- Timeout --//
		String timeout = Config.getFormatDateWithMiTz().print(new DateTime().plusSeconds(Config.getSdfCommitquotaTimeout()));
		...
		EStats statsOut = EStats.APP_SEND_SDF_PUT_W_SDF_COMMITQUOTALNFRA_REQUEST;
		...
		try {
			abstractAF.getEquinoxUtils().sendHTTPRequestMessage(builder, EEquinoxRawData.CTypeHTTP.TEXT_PLAIN, invoke,
					Config.getSdfCommitquotaInterface(), map);
		} catch (Exception e) {
